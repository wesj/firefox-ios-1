...
class FaviconSite {
    let site: Site
    let icon: SavedFavicon

    init(site: Site, icon: SavedFavicon) {
        self.site = site
        self.icon = icon
    }
}
...
class FaviconSiteTable: Table {
    ...
    func create(db: SQLiteDBConnection, version: Int) -> Bool {
        println("Creating favicon-sites table")
        ...
        println("Created favicon-sites table: CREATE TABLE IF NOT EXISTS \(name) (\(rows))")
        return true
    }
    ...
    private func updateFavicon(db: SQLiteDBConnection, icon: Favicon, inout err: NSError?) -> Int {
        // If our site doesn't have a guid, we need to find one
        if icon.guid == nil {
    ...
    func delete<T>(db: SQLiteDBConnection, item: T?, inout err: NSError?) -> Int {
        if let favicon = item as? Favicon {
