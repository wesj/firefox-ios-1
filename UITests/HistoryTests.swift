...
class HistoryTests: KIFTestCase {
    ...
    func testHistoryUI() {
        ...
        let firstHistoryRow = tester().waitForViewWithAccessibilityLabel("Page 1") as! UITableViewCell
        XCTAssertNotNil(firstHistoryRow.imageView?.image)
        let secondHistoryRow = tester().waitForViewWithAccessibilityLabel("Page 2") as! UITableViewCell
