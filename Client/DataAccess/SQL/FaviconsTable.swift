...
class FaviconsTable<T>: GenericTable<Favicon> {
    ...
    override func getInsertAndArgs<T>(item: T) -> (String?, [AnyObject?]) {
        let saved = item as SavedFavicon
        // Runtime errors happen if we let Swift try to infer the type of this array
        // so we construct it very specifically.
        var args = [AnyObject?]()
        if saved.favicon.guid == nil {
            saved.favicon.guid = NSUUID().UUIDString
        }
        args.append(saved.favicon.guid)
        args.append(saved.favicon.url)
        args.append(saved.filename)
        args.append(NSDate())
        if let image = saved.favicon.image {
            args.append(image.size.width)
        } else {
            args.append(0)
        }

        println("Inserting \(args)")
